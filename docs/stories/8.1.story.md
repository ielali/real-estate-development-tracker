...
1. [x] Notifications table created with proper schema and indexes
2. [x] Bell icon visible in header for all authenticated users
3. [x] Unread count badge displays correct number
4. [x] Notification panel opens on bell icon click
5. [x] Notifications grouped by date with clear sections
6. [x] Each notification shows icon, message, timestamp, read status
7. [x] Clicking notification navigates to entity and marks as read
8. [x] "Mark all as read" button works correctly
9. [x] New notifications appear within 30 seconds (polling interval)
10. [x] Notifications generated for all defined events
11. [x] Large expense threshold ($10k) triggers immediate notification
12. [x] Notification panel shows loading state during fetch
13. [x] Empty state displayed when no notifications
14. [x] Notifications respect RBAC (users only see their notifications)
15. [x] Notification cleanup job removes old notifications (>90 days)
...
- [ ] Implement notification navigation (AC: 7)
  ...
  - [ ] Map entityType to URL patterns:
    - [ ] 'cost' → `/projects/[projectId]/costs/[entityId]`
    - [ ] 'document' → `/projects/[projectId]/documents/[entityId]`
    - [ ] 'event' → `/projects/[projectId]/timeline/[entityId]`
...
- [ ] Write comprehensive tests (AC: 1-15)
  ...
  - [ ] Integration tests:
    - [ ] Test: Create cost → notification generated → appears in list
    - [ ] Test: Upload document → notification generated for all members
    - [ ] Test: Invite partner → notification sent to partner only
...
│   ├── server/
│   │   ├── db/schema/
│   │   │   └── notifications.ts           # New notification schema
│   │   ├── api/routers/
│   │   │   ├── notification.ts            # New notification router
│   │   │   ├── cost.ts                    # Modify: Add notification generation
│   │   │   ├── document.ts                # Modify: Add notification generation
│   │   │   ├── event.ts                   # Modify: Add notification generation
│   │   │   └── project.ts                 # Modify: Add notification for partner invite
│   │   ├── utils/
│   │   │   └── notifications.ts           # Notification generation utilities
│   │   └── jobs/
│   │       └── cleanup-notifications.ts   # Cleanup job (90-day expiry)
│   ├── components/
│   │   └── notifications/
│   │       ├── NotificationBell.tsx       # Bell icon + popover trigger
│   │       ├── NotificationPanel.tsx      # Notification list panel
│   │       ├── NotificationItem.tsx       # Individual notification component
│   │       └── __tests__/                 # Component tests
│   ├── lib/utils/
│   │   └── notification-navigation.ts     # Entity URL mapping
│   ├── app/api/cron/
...
function generateNotificationMessage(
  ...
  switch (type) {
    ...
    case "large_expense":
      return `🚨 ALERT: ${formatCurrency(params.amount!)} large expense added: ${params.entityName}`
...
- Cost creation → notification generated → appears in list
- Document upload → notifications for all project members
- Partner invitation → notification to invited user only
- Notification click → navigation + mark as read
...
4. Browser navigates to project page with `?tab=costs&costId=xyz` ✅
5. Project page reads query params and sets active tab to "costs" ✅
6. CostsList receives costId and highlights the specific cost ✅
7. Smooth scroll automatically centers the cost in viewport ✅
8. Notification panel closes ✅

**Testing**:

- ✅ 8 unit tests for notification-navigation utility (all passing)
- ✅ URL generation for all entity types verified
- ✅ Manual testing confirms end-to-end flow works correctly
- ✅ TypeScript compilation successful with no errors
- ✅ Visual highlighting appears correctly with primary theme colors
...
- AC #10 (Navigation URL generation): ✅ Implemented
- AC #11 (Click navigation with mark as read): ✅ Implemented
...
- ✅ Page navigates and re-renders with new query params
- ✅ Tab switches automatically to target tab
- ✅ Cost highlights with primary theme colors
- ✅ Smooth scroll centers cost in viewport
...
**Status**: ✅ Navigation fully functional as of commit `0ba104b`
...
📄 **Full Report**: [docs/qa/pre-implementation-reviews/8.1-notification-system.md](../../qa/pre-implementation-reviews/8.1-notification-system.md)
...
- ✅ Acceptance Criteria Quality: 9/10 (Excellent - all 15 ACs testable)
- ✅ Data Model Design: 9/10 (Excellent - proper schema, indexes, RBAC)
- ✅ Implementation Guidance: 10/10 (Excellent - production-ready code examples)
- ✅ Security Planning: 10/10 (Excellent - RBAC, XSS prevention, input validation)
- ✅ Test Strategy: 8/10 (Good - comprehensive, some edge cases missing)
- ✅ Performance Planning: 9/10 (Excellent - indexing, caching, polling optimization)
...
**Architectural Risks Identified** ⚠️:
...
2. **Implement Batch Insert for Notifications**
   ...
   - Generate 500 costs → 500 × N members notifications = 2,500+ inserts
...
- ✅ RBAC enforcement (User A cannot see User B's notifications)
- ✅ Notification generation for all 6 event types
- ✅ Large expense threshold ($10k = 1,000,000 cents)
- ✅ Mark as read + navigation flow
- ✅ Polling (notifications appear within 30 seconds)
- ✅ Date grouping (Today, Yesterday, This Week, Older)
- ✅ Cleanup job (deletes notifications >90 days)
...
- ✅ List query filters by `userId` (user sees only their notifications)
- ✅ Mark as read verifies ownership before update
- ✅ Generation restricted to project members with `acceptedAt != null`
- ✅ Partner invitations sent only to invited user

**XSS Prevention**:

- ✅ React automatically escapes JSX content (no dangerouslySetInnerHTML)
- ✅ Drizzle ORM uses parameterized queries (SQL injection prevented)
- ⚠️ Recommendation: Add Zod validation for notification message parameters
...
- 100 concurrent users × 2 requests (list + unread) / 30s = ~400 QPM
- **Assessment**: Moderate load, within PostgreSQL capacity

**Index Coverage**:

- ✅ `idx_notifications_user_read` - Covers list + unread count queries
- ✅ `idx_notifications_created_at` - Covers cleanup job
- ⚠️ `idx_notifications_user_id` - May be redundant (composite index covers prefix)
...
| ------------------------------------- | ---------- | ------ | ---------- | --------------------------- |
| Polling overloads database            | Medium     | High   | **HIGH**   | Indexes, caching, limits ✅ |
| Notification failures break parent TX | Medium     | High   | **HIGH**   | Fire-and-forget pattern ⚠️  |
| Cleanup job locks table               | Low        | Medium | **MEDIUM** | Chunked deletes ⚠️          |
| 30s polling E2E test flaky            | High       | Low    | **MEDIUM** | Test timing controls ⚠️     |
| Orphaned notifications                | Medium     | Low    | **LOW**    | Display gracefully ✅       |
| XSS via notification messages         | Low        | High   | **MEDIUM** | React escaping verified ✅  |
...
- ✅ Notification navigation utility (`notification-navigation.ts`)
- ✅ Unit tests for navigation utility (8 tests passing)
- ✅ Integration with existing UI (CostsList highlighting, tab switching)

**What Remains To Be Implemented** (13 of 15 ACs):

- ❌ AC #1: Notifications table/schema/indexes
- ❌ AC #2: Bell icon in header
- ❌ AC #3: Unread count badge
- ❌ AC #4: Notification panel opens on click
- ❌ AC #5: Notifications grouped by date
- ❌ AC #6: Notification display (icon/message/timestamp/read)
- ⚠️ AC #7: Click navigation + mark as read (PARTIALLY - navigation only, no mark as read)
- ❌ AC #8: "Mark all as read" button
- ❌ AC #9: New notifications appear within 30s (polling)
- ❌ AC #10: Notifications for all defined events
- ❌ AC #11: Large expense threshold ($10k)
- ❌ AC #12: Loading state
- ❌ AC #13: Empty state
- ❌ AC #14: RBAC enforcement
- ❌ AC #15: Cleanup job (>90 days)
...
- ✅ Type-safe interface (`NotificationNavigationParams`)
- ✅ Comprehensive test coverage (8 tests, all passing)
- ✅ Handles all entity types (cost, document, event, project)
- ✅ Proper fallback behavior (unknown types, missing projectId)
- ✅ Query parameter approach for cost highlighting
- ✅ Follows project coding standards

**Test Coverage**:

```
✅ Cost notification URL generation
✅ Document notification URL generation
✅ Event notification URL generation
✅ Project notification URL generation
✅ Unknown entity type fallback
✅ Null projectId handling
✅ isNavigableNotification() helper
✅ Edge cases covered
```

**Integration Quality**:

- ✅ Integrates with CostsList component (highlighting + smooth scroll)
- ✅ Integrates with Project page (controlled tabs)
- ✅ Uses hard navigation (`window.location.href`) for reliability
- ✅ Query parameters approach maintains route structure

**Technical Decisions** (from Dev Agent Record):

1. ✅ Query parameter approach (`?tab=costs&costId=xyz`) - Good choice for flexibility
2. ✅ Controlled tabs pattern - Correct for programmatic tab switching
3. ✅ 100ms scroll delay - Prevents race conditions
4. ✅ Single ref for highlighted cost - Performance optimization
5. ✅ Primary theme colors - Design system consistency
6. ✅ Hard navigation - Necessary workaround for Next.js App Router limitations
...
**AC #7**: ⚠️ **PARTIALLY COMPLETE**

- ✅ Navigation URL generation implemented
- ✅ Navigation to entity works (cost highlighting, tab switching)
- ❌ "Mark as read" functionality NOT implemented (no backend, no database)

**All Other ACs (1-6, 8-15)**: ❌ **NOT IMPLEMENTED**
...
**Gate**: ⚠️ **NOT APPLICABLE** (Partial implementation - full review pending)
...
- ✅ Navigation utility approved (production-ready)
- ⚠️ Full notification system implementation still required
- ⚠️ Story should remain in "Approved" status (not "Complete" or "Review")
- ⚠️ Request post-implementation QA review after completing all 15 ACs
...
### Comprehensive Implementation Review (2025-11-03)

**Review Date**: 2025-11-03
**Reviewed By**: Bob (Scrum Master) - Code Review
**Review Type**: Comprehensive Codebase Analysis

#### Implementation Status

**✅ IMPLEMENTATION COMPLETE** (All 15 ACs Implemented)

The previous QA assessment significantly underestimated the implementation progress. A comprehensive code review reveals that **all 15 acceptance criteria have been fully implemented**.

**What Was Actually Implemented**:

#### Backend Implementation (100% Complete)

1. ✅ **Database Schema** ([apps/web/src/server/db/schema/notifications.ts](../../../apps/web/src/server/db/schema/notifications.ts)):
   - Complete notification table with all required fields
   - 4 indexes for query optimization (user_id, user_id+read, created_at, project_id)
   - Type definitions and enums for NotificationType and EntityType
   - Relations configured for users and projects
   - AC #1: ✅ COMPLETE

2. ✅ **Notification Services** ([apps/web/src/server/services/notifications.ts](../../../apps/web/src/server/services/notifications.ts)):
   - `createNotification()` - Single notification creation
   - `notifyProjectMembers()` - Batch notification for all project members
   - Convenience functions: `notifyCostAdded()`, `notifyLargeExpense()`, `notifyDocumentUploaded()`, `notifyTimelineEvent()`, `notifyPartnerInvited()`
   - Message templates for all 6 notification types
   - Excludes acting user from notifications
   - AC #10: ✅ COMPLETE

3. ✅ **tRPC Notification Router** ([apps/web/src/server/api/routers/notification.ts](../../../apps/web/src/server/api/routers/notification.ts)):
   - `list` query - Paginated notifications with filtering (unread, project)
   - `getUnreadCount` query - Badge count
   - `markAsRead` mutation - Single notification
   - `markAllAsRead` mutation - Batch update
   - RBAC enforcement at query level (userId filtering)
   - Registered in root router
   - AC #7, #8, #9, #14: ✅ COMPLETE

4. ✅ **Integration into Existing Routers**:
   - **Cost Router** ([apps/web/src/server/api/routers/cost.ts](../../../apps/web/src/server/api/routers/cost.ts)):
     - Integrated notification generation after cost creation
     - Large expense threshold detection ($10k = 1,000,000 cents)
     - Calls `notifyLargeExpense()` or `notifyCostAdded()` based on amount
     - AC #10, #11: ✅ COMPLETE

   - **Document Router** ([apps/web/src/server/api/routers/documents.ts](../../../apps/web/src/server/api/routers/documents.ts)):
     - Integrated notification generation after document upload
     - Calls `notifyDocumentUploaded()` with project members
     - AC #10: ✅ COMPLETE

   - **Event Router** ([apps/web/src/server/api/routers/events.ts](../../../apps/web/src/server/api/routers/events.ts)):
     - Integrated notification generation after timeline event creation
     - Calls `notifyTimelineEvent()` with project members
     - AC #10: ✅ COMPLETE

   - **Partner Router** ([apps/web/src/server/api/routers/partners.ts](../../../apps/web/src/server/api/routers/partners.ts)):
     - Integrated notification generation after partner invitation
     - Calls `notifyPartnerInvited()` for invited user only
     - AC #10: ✅ COMPLETE

5. ✅ **Cleanup Job** ([apps/web/scripts/cleanup-notifications.ts](../../../apps/web/scripts/cleanup-notifications.ts)):
   - Manual script with dry-run mode
...
1. ✅ **NotificationBell Component** ([apps/web/src/components/notifications/NotificationBell.tsx](../../../apps/web/src/components/notifications/NotificationBell.tsx)):
   - Bell icon button with proper sizing (lucide-react)
   - Unread count badge (destructive variant, "99+" for >99)
   - Popover trigger for notification panel
   - 30-second polling for unread count
   - Accessible ARIA labels
   - AC #2, #3, #4: ✅ COMPLETE

2. ✅ **NotificationPanel Component** ([apps/web/src/components/notifications/NotificationPanel.tsx](../../../apps/web/src/components/notifications/NotificationPanel.tsx)):
   - Grouped notifications by date (Today, Yesterday, This Week, Older)
   - Each notification displays: icon (6 types), message, timestamp, read status
   - Click handler: marks as read + navigates to entity
   - "Mark all as read" button with optimistic updates
   - Loading state with message
   - Empty state with icon and helpful text
   - 30-second polling for notification list
   - Smooth scrollable area with proper styling
   - AC #5, #6, #7, #8, #9, #12, #13: ✅ COMPLETE

3. ✅ **Navbar Integration** ([apps/web/src/components/layout/Navbar.tsx](../../../apps/web/src/components/layout/Navbar.tsx)):
   - NotificationBell imported and rendered in header
   - Positioned between Search button and UserDropdown (line 98)
   - Only visible for authenticated users
   - Responsive design (mobile + desktop)
   - AC #2: ✅ COMPLETE

4. ✅ **Navigation Utility** ([apps/web/src/lib/utils/notification-navigation.ts](../../../apps/web/src/lib/utils/notification-navigation.ts)):
   - `getNotificationNavigationUrl()` - URL generation for all entity types
   - Maps cost/document/event/project to correct URLs
   - Uses query parameters for cost highlighting (`?tab=costs&costId=xyz`)
   - Hard navigation for reliable page reloads
   - 8 unit tests (all passing)
   - AC #7: ✅ COMPLETE

5. ✅ **Enhanced Project Page & CostsList**:
   - Project page uses controlled tabs with URL parameter support
   - CostsList highlights target cost with smooth scroll
   - Visual feedback with primary theme colors
   - 100ms scroll delay for DOM stability
   - AC #7: ✅ COMPLETE
...
- ✅ Navigation utility: 8 tests (all entity types, edge cases)

**Missing Tests** (Noted for Future Enhancement):

- ⚠️ Backend router tests (notification.ts router)
- ⚠️ Notification service tests (notifications.ts)
- ⚠️ Component tests (NotificationBell, NotificationPanel)
- ⚠️ Integration tests (end-to-end notification flow)
- ⚠️ E2E tests (Playwright)

**Test Coverage**: ~5% (8 tests vs target of 70-100)

#### RBAC Enforcement Verification

✅ **Confirmed** - RBAC implemented correctly:

- `list` query filters by `ctx.session.user.id` (line 34)
- `markAsRead` mutation verifies ownership before update (line 109)
- `markAllAsRead` mutation scoped to user's notifications (line 124)
- Notification generation excludes acting user via `excludeUserId`
- Partner notifications sent only to invited user

#### Performance Considerations

✅ **Polling Strategy**:

- 30-second interval for both unread count and notification list
- Background polling enabled (continues when tab inactive)
- Window focus refetch enabled
- Stale time: 25 seconds (prevents redundant fetches)

✅ **Database Indexes**:

- `notifications_user_id_idx` - User query optimization
- `notifications_user_id_read_idx` - Unread count optimization
- `notifications_created_at_idx` - Cleanup job optimization
- `notifications_project_id_idx` - Project filtering optimization

#### Git Commit History

Comprehensive implementation across 14 commits:

- `b7fa1c8` - feat: add notifications database schema and migration
- `621baa7` - feat: add tRPC notification router and API endpoints
- `b94b5a9` - feat: add notification generation utilities
- `d4fe0d1` - feat: integrate notifications into mutation endpoints
- `0fd954d` - feat: add notification UI components
- `e23270e` - feat: integrate NotificationBell into main app layout
- `6655fbc` - feat: implement notification cleanup job
- `3c4f27c` - fix: implement notification navigation
- `b4f14fe` - feat: implement automatic tab switching and cost highlighting
- `0ba104b` - fix: use hard navigation for notification clicks
- `d92839c` - docs: update story with implementation details
- `c72bd3b` - docs: update story with hard navigation fix details
- `1480e68` - docs: add pre-implementation QA review
- `a7e921b` - Merge feature/story-8.1-notifications into main
...
1. ✅ Text-based enums instead of pgEnum (flexibility)
2. ✅ Hard navigation for reliability (Next.js App Router limitation workaround)
3. ✅ Batch insert for notification generation (performance)
4. ✅ Query parameter approach for navigation (maintains route structure)
5. ✅ Controlled tabs pattern (programmatic control)
6. ✅ Single ref for highlighted element (performance optimization)
...
| --- | -------------------------------------- | ----------- | --------------------------------- |
| 1   | Notifications table + schema + indexes | ✅ COMPLETE | notifications.ts schema           |
| 2   | Bell icon in header                    | ✅ COMPLETE | Navbar.tsx line 98                |
| 3   | Unread count badge                     | ✅ COMPLETE | NotificationBell.tsx              |
| 4   | Panel opens on click                   | ✅ COMPLETE | Popover in NotificationBell       |
| 5   | Grouped by date                        | ✅ COMPLETE | NotificationPanel grouping        |
| 6   | Shows icon/message/timestamp/read      | ✅ COMPLETE | NotificationPanel display         |
| 7   | Click navigates + marks read           | ✅ COMPLETE | NotificationPanel + navigation.ts |
| 8   | "Mark all as read" button              | ✅ COMPLETE | NotificationPanel                 |
| 9   | Polling (30s)                          | ✅ COMPLETE | React Query config                |
| 10  | Notifications for all events           | ✅ COMPLETE | All 4 routers integrated          |
| 11  | Large expense threshold ($10k)         | ✅ COMPLETE | Cost router threshold check       |
| 12  | Loading state                          | ✅ COMPLETE | NotificationPanel                 |
| 13  | Empty state                            | ✅ COMPLETE | NotificationPanel                 |
| 14  | RBAC enforcement                       | ✅ COMPLETE | notification.ts router            |
| 15  | Cleanup job (>90 days)                 | ✅ COMPLETE | cleanup-notifications.ts          |

**Overall Completion**: 15/15 ACs ✅ **100% COMPLETE**

#### Recommended Next Steps

**Before Marking "Done"**:

1. ⚠️ Add comprehensive backend tests (30-40 tests)
2. ⚠️ Add component tests for NotificationBell/Panel (20-30 tests)
3. ⚠️ Add integration tests (15-20 tests)
4. ⚠️ Add E2E tests (5-8 tests)
5. ⚠️ Verify database migration applied to production
6. ⚠️ Manual QA testing of all 15 ACs in staging environment
...
- ✅ Code quality excellent
- ✅ Security verified
- ⚠️ Test coverage low (5% vs target 70-100%)
- ⚠️ No E2E validation

**Recommendation**:
Story can be marked as **"Review"** for final QA validation. The implementation is functionally complete but would benefit from comprehensive test coverage before production deployment.

---

**Comprehensive Implementation Review Completed**: 2025-11-03
**Status**: Ready for final QA validation and test coverage enhancement
