...
2. [ ] CSV upload accepts .csv and .txt files
3. [ ] CSV parser handles common formats (comma, tab, semicolon delimited)
4. [ ] Preview shows first 10 rows with column mapping interface
5. [ ] User can map CSV columns to database fields
6. [ ] Validation shows errors with line numbers
7. [ ] Category dropdown shows existing categories for mapping
8. [ ] Vendor matching works by name (case-insensitive)
9. [ ] Option to create new vendors from import
10. [ ] All-or-nothing import (transaction rolls back on any error)
11. [ ] Success message shows count of imported costs
12. [ ] Error report downloadable if validation fails
13. [ ] Import tested with 500+ cost entries
14. [ ] Import tested with various date formats
15. [ ] Import tested with special characters and unicode
16. [ ] Import activity logged for audit trail
...
- [ ] Create Zod validation schema for cost import (AC: 6, 14, 15)
  - [ ] Create `apps/web/src/lib/validations/cost-import.ts`
  - [ ] Define `importCostRowSchema` with field validation:
    - [ ] Required: date (multiple formats), description (string), amount (number), category (string)
    - [ ] Optional: vendor (string), notes (string)
  - [ ] Implement date format detection (MM/DD/YYYY, YYYY-MM-DD, DD/MM/YYYY, etc.)
  - [ ] Validate amount accepts decimals and currency symbols ($, commas)
  - [ ] Validate special characters and unicode in description/notes
...
- [x] Create tRPC import endpoints with transaction support (AC: 10, 13, 16)
  ...
  - [ ] Unit tests for both procedures (success, validation errors, transaction rollback)

- [ ] Create CostImport UI component with file upload (AC: 1, 2)
  - [ ] Create `apps/web/src/components/costs/CostImport.tsx`
  - [ ] Add "Import Costs" button to cost list toolbar
  - [ ] Implement file upload dialog (Shadcn/ui Dialog + Input)
  - [ ] Accept .csv and .txt files (validate MIME type client-side)
  - [ ] Parse uploaded file using CSV parser utility
  - [ ] Display file info (name, size, row count)
  - [ ] Component tests for file upload and parsing

- [ ] Create ColumnMapping component for field mapping (AC: 4, 5, 7)
  - [ ] Create `apps/web/src/components/costs/ColumnMapping.tsx`
  - [ ] Display preview of first 10 CSV rows in table format
  - [ ] Show dropdown for each CSV column to map to database fields:
    - [ ] Database field options: Date, Description, Amount, Category, Vendor, Notes, Skip
    - [ ] Detect likely mappings based on column headers (e.g., "date" → Date)
    - [ ] Highlight required field mappings (Date, Description, Amount, Category)
  - [ ] Category mapping: Dropdown with existing categories (fetch from categories table)
    - [ ] Allow "Create New Category" option (if category name not found)
    - [ ] Show category hierarchy in dropdown (parent > child)
  - [ ] Vendor mapping display:
    - [ ] Show matched vendors with confidence score (exact match, fuzzy match)
    - [ ] Show unmatched vendor names with "Create New" checkbox
  - [ ] Validation indicator: Show which columns are required vs optional
  - [ ] Component tests for mapping interface and validation

- [ ] Create ImportPreview component with validation display (AC: 6, 12)
  - [ ] Create `apps/web/src/components/costs/ImportPreview.tsx`
  - [ ] Call `validateImport` tRPC procedure with mapped data
  - [ ] Display validation summary:
    - [ ] Total rows, valid rows, error rows
    - [ ] List of validation errors with line numbers and field names
    - [ ] Error severity (blocker vs warning)
  - [ ] Show error report table (sortable and filterable)
  - [ ] "Download Error Report" button generates CSV with errors
    - [ ] Columns: Line Number, Field, Error Message, Value
    - [ ] Download as `import-errors-{timestamp}.csv`
  - [ ] "Fix Errors" button returns to column mapping
  - [ ] "Proceed with Import" button (disabled if errors exist)
  - [ ] Component tests for validation display and error download

- [ ] Create ImportConfirmation component with vendor creation (AC: 9, 10, 11)
  - [ ] Create `apps/web/src/components/costs/ImportConfirmation.tsx`
  - [ ] Display import summary before execution:
    - [ ] X costs will be imported
    - [ ] Y new vendors will be created (list names)
    - [ ] Category breakdown (count by category)
  - [ ] Checkbox: "Create new vendors from import" (default: true)
  - [ ] Warning message: "This operation cannot be undone. All costs will be imported or none."
  - [ ] "Confirm Import" button triggers `importCosts` mutation
  - [ ] Loading state during import (show progress if possible)
  - [ ] Success message: "Successfully imported X costs"
  - [ ] Error handling: Display error message, keep dialog open, suggest retry
  - [ ] Component tests for confirmation and execution

- [ ] Implement import workflow orchestration (AC: 1-16)
  - [ ] Create stepper component for multi-step import process:
    - [ ] Step 1: Upload CSV file
    - [ ] Step 2: Map columns to fields
    - [ ] Step 3: Preview and validate
    - [ ] Step 4: Confirm and import
  - [ ] Handle file format detection and errors
  - [ ] Handle validation errors and re-mapping
  - [ ] Handle import success and navigation back to cost list
  - [ ] Clear import state on dialog close
  - [ ] Integration tests for complete import workflow

- [ ] Add audit logging for import activity (AC: 16)
  - [ ] Log import start (user, timestamp, file name, row count)
  - [ ] Log import completion (success, imported count, created vendors)
  - [ ] Log import failure (error message, failed row count)
  - [ ] Display import activity in project activity feed
  - [ ] Include import metadata (file name, cost count) in audit log

- [ ] Write comprehensive tests (AC: 13, 14, 15)
  - [ ] Unit tests: `apps/web/src/lib/__tests__/csv-parser.test.ts`
    - [ ] Test delimiter detection (comma, tab, semicolon)
    - [ ] Test quoted field parsing
    - [ ] Test special characters (unicode, emojis, newlines in quoted fields)
    - [ ] Test malformed CSV handling
  - [ ] Unit tests: `apps/web/src/lib/validations/__tests__/cost-import.test.ts`
    - [ ] Test date format parsing (MM/DD/YYYY, YYYY-MM-DD, DD/MM/YYYY, ISO 8601)
    - [ ] Test amount parsing ($1,500.00, 1500, 1.500,00)
    - [ ] Test special characters in description/notes
    - [ ] Test unicode characters (Chinese, Arabic, emojis)
...
- tRPC import endpoints (validateImport, importCosts procedures)
- UI components (CostImport, ColumnMapping, ImportPreview, ImportConfirmation)
- Import workflow orchestration (stepper component)
- Audit logging integration
- Component and integration tests
- Performance optimization for 500+ cost imports
...
- apps/web/src/lib/**tests**/csv-parser.test.ts
- apps/web/src/lib/validations/cost-import.ts
