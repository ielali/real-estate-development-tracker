...
- [ ] Create partner dashboard tRPC router (AC: 2, 3, 4, 5)
  - [ ] Create `apps/web/src/server/api/routers/partnerDashboard.ts`
  - [ ] Add `getProjectSummary` query returning project with key metrics
    - [ ] Total spent by category (cost breakdown)
    - [ ] Recent activity from audit logs (last 30 days)
    - [ ] Document count by category
    - [ ] Project status and timeline info
  - [ ] Add `getCostBreakdown` query for category visualization data
    - [ ] Group costs by categoryId with totals
    - [ ] Calculate percentage of total spend per category
    - [ ] Include category display names from Category system
  - [ ] Add `getRecentActivity` query for timeline view
    - [ ] Fetch audit logs filtered by project access
    - [ ] Include user names, formatted timestamps, and action descriptions
    - [ ] Paginated results (default 20, max 50)
  - [ ] Add `getDocumentGallery` query for document viewing
    - [ ] Fetch documents with thumbnails grouped by category
    - [ ] Include file metadata (name, size, upload date, category)
    - [ ] Generate signed URLs for secure document access
  - [ ] All queries use `verifyProjectAccess()` from authorization.ts for security
  - [ ] All queries respect soft-delete filtering (deletedAt IS NULL)
